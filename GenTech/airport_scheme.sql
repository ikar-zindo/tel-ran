...
CREATE TABLE `Airplanes` (
	`id` INT NOT NULL AUTO_INCREMENT,
	`model` TEXT NOT NULL AUTO_INCREMENT,
	`range` INT NOT NULL AUTO_INCREMENT,
	`range` INT NOT NULL AUTO_INCREMENT,
	PRIMARY KEY (`id`)
);
...
ALTER TABLE `Tickets` ADD CONSTRAINT `Tickets_fk3` FOREIGN KEY (`seat_num`) REFERENCES `Seats`(`seat_num`);

ALTER TABLE `Flights` ADD CONSTRAINT `Flights_fk0` FOREIGN KEY (`airplane_id`) REFERENCES `Airplanes`(`id`);

ALTER TABLE `Flights` ADD CONSTRAINT `Flights_fk1` FOREIGN KEY (`airport_arrival`) REFERENCES `Airports`(`id`);

ALTER TABLE `Flights` ADD CONSTRAINT `Flights_fk2` FOREIGN KEY (`airport_departure`) REFERENCES `Airports`(`id`);
